...
class SVC(BinaryClassifier):
    ...
    def _train_epoch(self, epoch: int) -> EpochLog:
        ...
        objective = np.sum(self._alphas) - 0.5 * np.sum(
            np.outer(self._alphas, self._alphas) * kernel_X_X
        )
        return EpochLog(epoch, objective)
    ...
    def plot_decision_boundary(self):
        plt.scatter(self._X[:, 0], self._X[:, 1], c=self._y, s=50, alpha=0.5)
        ax = plt.gca()
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        ...
        ax.scatter(
            self._X[:, 0][self._alphas > 0.0],
            self._X[:, 1][self._alphas > 0.0],
            s=50,
            linewidth=1,
            facecolors="none",
            edgecolors="k",
